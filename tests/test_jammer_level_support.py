...
class TestRfProfiler:
    ...
    def test_semi_auto_reduces_gain_when_too_hot(self):
        import numpy as np
        import time
        p = RfProfiler(gain_db=30, gain_mode="semi_auto", window_s=0.0)
        iq = np.ones(1000, dtype=np.complex64) * 0.8  # > target_hi 0.50
        delta = p.update(iq)
        assert delta == -5
        assert p.gain_db == 25
    ...
    def test_semi_auto_increases_gain_when_too_cold(self):
        import numpy as np
        p = RfProfiler(gain_db=30, gain_mode="semi_auto", window_s=0.0)
        iq = np.ones(1000, dtype=np.complex64) * 0.01  # < target_lo 0.05
        delta = p.update(iq)
        # No frames decoded yet, so frames_s < 0.1 → increase gain
        assert delta == +5
        assert p.gain_db == 35

    def test_semi_auto_clamped(self):
        import numpy as np
        p = RfProfiler(gain_db=48, gain_mode="semi_auto", window_s=0.0,
