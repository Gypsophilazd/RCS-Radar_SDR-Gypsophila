...
解码帧示例：
```
[12:34:56.789] {"cmd": "0x0A06", "key": "Fuckyo", "seq": 1}
```
...
**当前工程实践**：虽然 AD9363 模拟 DAC 本体是 12-bit，但 `pyadi-iio` 走的是 Pluto 的 16-bit DMA 数据通道。为获得稳定且足够强的发射功率，当前代码统一使用 `2^14 × 0.5 = 8192`（约 `-6 dBFS`），这也是 `rx_pluto_pipeline.py` 与 `tx_signal_produce.py` 现已对齐的实现。
