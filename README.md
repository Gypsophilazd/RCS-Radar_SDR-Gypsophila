...
RCS-Radar-SDR/
├── main.py                  # 系统入口，串联所有模块
├── config.json              # 赛场配置文件（队伍颜色、干扰等级、增益等）
│
├── config_manager.py        # Module 1: 频率计划解析（自动计算 LO / 采样率）
├── tx_signal_produce.py     # Module 2: PlutoSDR TX 信号生成
├── rx_sdr_driver.py         # Module 3: PlutoSDR RX 驱动（MGC 增益控制）
├── dsp_processor.py         # Module 4a: 数字信道化 + 解调 + 符号恢复（全有状态滤波）
├── packet_decoder.py        # Module 4b: 协议帧解析（0x0A01–0x0A06）
├── visual_terminal.py       # Module 5: pyqtgraph 实时四面板仪表盘
│
├── rx_4fsk_pipeline.py      # 参考脚本：软件仿真 + RTL-SDR 硬件（已验证，保留）
├── rx_pluto_pipeline.py     # 参考脚本：双 Pluto 硬件全链路（已验证，保留）
└── fsk_digital_twin.py      # 内部库：协议帧构建（被 tx_signal_produce.py 调用）
```
...
| 设备 | 型号 | URI 示例 | 说明 |
|---|---|---|---|
| TX Pluto | ADALM-PLUTO | `usb:1.15.5` | 仅发射；`tx_hardwaregain_chan0 = 0 dB`（最大功率） |
| RX Pluto | ADALM-PLUTO | `usb:1.16.5` 或 `ip:pluto.local` | 搭配滤波器+放大器 |
| 带通滤波器 | 433 MHz SAW/LC | — | **必须**串接在 RX 天线口，否则带外干扰导致解调失败 |
| LNA/放大器 | 433 MHz 低噪声放大器 | — | 可选；显著提升远距离 SNR |
| 天线 TX | 433 MHz 1/4 波长 | SMA | — |
| 天线 RX | 433 MHz | SMA | — |

> **Linux RX 部署**：RX Pluto 可部署在 Linux 主机上运行 `rx_pluto_pipeline.py`，无需 GUI 依赖（`--hw --rx-only`）。

### 单 Pluto / RTL-SDR（参考 / 调试用）

| 设备 | 说明 |
|---|---|
| 单 ADALM-PLUTO | TX + RX 共用同一块板，适合软件回环测试 |
| RTL-SDR Blog V4 | 仅用 `rx_4fsk_pipeline.py --hw`；Windows 需 `librtlsdr.dll` |

**Windows 额外依赖**（放在脚本同目录）：
- `librtlsdr.dll` + `libusb-1.0.dll`（仅用 RTL-SDR 时需要）
- IIO Oscilloscope（含 `libiio`，使用 Pluto 必须安装）
...
# 核心包：numpy scipy pyadi-iio pyrtlsdr PyQt5 pyqtgraph
```
linux 下需要执行命令：
...
```bash
cd D:\CODE\RCS-Radar-SDR
python rx_4fsk_pipeline.py
```

预期：
```
[PASS] Key correctly recovered (4998 frame(s) decoded).
```

仿真条件：+5000 Hz 载波偏移、15 dB SNR、分块大小 262144，与真机完全一致。
...
打开三面板仪表盘，使用合成数据驱动，无需任何 SDR 硬件。

### 步骤 3：硬件全链路

```bash
# 编辑 config.json，设置 team_color 和 target_jammer_level
python main.py
```

Pluto TX 自动广播，RX 开始解调，仪表盘实时更新。按 `Ctrl-C` 停止。

### 步骤 4（旧接口）：单体脚本硬件测试

```bash
python rx_4fsk_pipeline.py --hw
```

适用于快速验证硬件连通性，解码帧直接打印到终端：
```
...
```bash
# 命令行临时覆盖
python main.py --key "ABCDEF"
python rx_4fsk_pipeline.py --hw --key "ABCDEF"
```

若要固定更改，编辑 `fsk_digital_twin.py` 中 `build_protocol_frame()` 的默认参数，或在 `config.json` 中添加 `"key"` 字段（`main.py` 会读取）。
...
```json
{
  "team_color":           "red",        // "red" 或 "blue" — 我方颜色
  "target_jammer_level":  1,            // 0~3，对方当前干扰等级
  "pluto_uri":            "usb:1.15.5", // TX Pluto URI（每次重插会变，先用 iio_info -s 查）
  "pluto_rx_uri":         "usb:1.16.5", // RX Pluto URI（同上；双 Pluto 时必填）
  "rx_gain_db":           30,           // RX 手动增益（见第 7 节调参方法）
  "tx_attenuation_db":    0,            // TX 衰减（0 = 最大功率，-89 = 最小）
  "rx_buf_size":          262144        // 每次回调样本数（必须是 2 的幂）
}
```

> **URI 会随 USB 热插拔改变**。每次重新连接设备后，运行以下命令确认当前 URI：
> ```python
...
目标：IQ RMS 在 **0.05 ~ 0.30** 之间。

```bash
# 1. 抓取 2 秒 IQ（使用旧接口）
python rx_4fsk_pipeline.py --hw --capture

# 2. 离线诊断
python rx_4fsk_pipeline.py --diagnose
```
...
> **赛场注意**：+10 dBm 干扰机比 −60 dBm 广播强约 70 dB。若两个信道同时在带内，需将增益设置得更保守（约 20 dB），然后依赖数字信道化滤波器分离两个信号。
...
| 命令 | 说明 |
|---|---|
| `python main.py` | 全硬件模式（TX + RX + GUI） |
| `python main.py --rx-only` | 仅 RX + DSP + GUI |
| `python main.py --tx-only` | 仅 TX |
| `python main.py --key MYKEY` | 指定密钥 |
| `python main.py --no-gui` | 无 GUI 仅打印帧 |
| `python main.py --demo` | 合成数据演示（无硬件） |
| `python visual_terminal.py` | 单独启动 GUI 演示 |
| `python config_manager.py` | 打印当前频率计划 |
| `python packet_decoder.py` | CRC 轮回测试 |
...
| 命令 | 说明 |
|---|---|
| `python rx_4fsk_pipeline.py` | 软件仿真 loopback（**无需硬件**） |
| `python rx_4fsk_pipeline.py --hw` | RTL-SDR 硬件全链路 |
| `python rx_4fsk_pipeline.py --hw --capture` | 抓取 IQ 到 `iq_capture.npy` |
| `python rx_4fsk_pipeline.py --diagnose` | 离线诊断 `iq_capture.npy` |
| `python rx_4fsk_pipeline.py --hw --rx-only` | 仅 RX |
| `python rx_4fsk_pipeline.py --hw --tx-only` | 仅 TX |

#### `rx_pluto_pipeline.py`（双 Pluto 硬件全链路）

| 命令 | 说明 |
|---|---|
| `python rx_pluto_pipeline.py` | 软件回环自测（无需硬件） |
| `python rx_pluto_pipeline.py --hw` | TX Pluto + RX Pluto 全链路 |
| `python rx_pluto_pipeline.py --hw --rx-only` | 仅 RX Pluto（外部发射机模式） |
| `python rx_pluto_pipeline.py --hw --tx-only` | 仅 TX Pluto |
| `python rx_pluto_pipeline.py --hw --key ABCDEF` | 指定密钥 |
| `python rx_pluto_pipeline.py --hw --gain 40` | 指定 RX 增益 |
...
| 有信号但 CRC 全失败 | TX 发送了 `int16` 而非 `complex64` | 见第 15 节 Pluto TX 格式 |
| 符号分布严重偏向 `−3` | 信号幅度被压缩，MF 未对齐 | 先降增益，再检查 Gardner TED 是否锁定 |
| `[PLUTO TX failed: [Errno 0]` | libIIO DMA 缓冲未释放 | 等 3 秒重试，或拔插 USB |
| ping 通 `192.168.2.1` 但 pyadi-iio 找不到设备 | IIO Oscilloscope 未安装或版本不对 | 安装随 Pluto 配套的 IIO Oscilloscope |
| 干扰机在带内，广播帧丢包 | AGC 被干扰机拉高，压制广播 | 启用 MGC（`rx_sdr_driver.py` 已默认 MGC），增益适度保守 |
...
正常符号分布参考：`−3: ~46.7%  −1: ~20.0%  +1: ~23.3%  +3: ~10.0%`（高斯信道下的统计期望）
...
PlutoTxProducer ──► 4-RRC-FSK IQ ──► Pluto TX ──► 射频空间       │
                                                        │         │
                                              433 MHz   ▼         │
PlutoRxDriver   ◄── 复数 IQ 流  ◄── Pluto/RTL-SDR RX ──┘         │
    │                                                             │
...
发射方晶振与接收方晶振存在频偏，例如 RTL-SDR 的典型误差为 **±20 ppm**。在 2 MSPS 下，这意味着每秒漂移 **±40 个样本**。若不校正，接收眼图在几百毫秒后就会完全闭合。
...
1. 取当前子块的 RRC MF 输出
2. 在 0 ~ SPS−1 共 8 个候选相位上，计算每个相位下抽取样本的**能量**（$\sum |y|^2$）
...
真实载波偏移（Pluto vs RTL-SDR 44.1 kHz TCXO）约 **4 500 Hz**。判决阈值容限为：
...
```python
# pyadi-iio 要求 complex64；AD9363 是 12-bit DAC，满量程 = ±2047 计数
iq = np.exp(1j * phase_array).astype(np.complex64) * 2047.0
sdr.tx(iq)
```

**根本原因**：`adi.Pluto.tx()` 内部将 `complex64` 拆分为两路 `int16` 写入 DMA。若用户直接传入 `int16`，pyadi-iio 会将其字节流重新解读为 `float32`，导致每个样本的 I/Q 值完全错误。

**幅度系数**：AD9363 为 **12-bit DAC**，满量程为 ±2047（非 ±2^14）。代码中使用 `DAC_FULL_SCALE = 2047`。
...
Offset  Size  字段       说明
0       1     SOF        0xA5（帧起始标志）
1       2     Length     uint16 LE，总帧字节数（含头尾）
3       1     Seq        序号（0~255 滚动）
4       1     CmdID_H    命令 ID 高字节
```

帧尾：`CRC8 (1 byte)` + `CRC16 (2 bytes)`，共 3 字节。

### 支持的命令 ID

| CmdID | 名称 | Payload 字节数 | 说明 |
|---|---|---|---|
| `0x0A01` | 单位坐标 | 24 | 5 个单位 × (x_cm uint16 + y_cm uint16) + 有效掩码 |
| `0x0A02` | 单位 HP | 12 | 6 个单位 × HP uint16 |
| `0x0A03` | 弹药余量 | 10 | 5 个射手 × 弹药 uint16 |
| `0x0A04` | 资源状态 | 8 | 金额 uint16 + 前哨 HP uint16 + 占区掩码 uint32 |
| `0x0A05` | Buff 状态 | 36 | 6 单位 × 6 buff字段 × uint8 |
| `0x0A06` | 密钥 | 6 | char[6] ASCII，不足补 `\x00` |
...
│  └──┘  │  │  └──┘  └──────────────┘     └──┘
SOF Len  Seq │  └─CmdID=0x0A06(LE)─┘     CRC16
             CRC8（覆盖 SOF+Len+Seq）
```
...
|---|---|---|---|---|---|
| Red | L0（无干扰） | 广播 only | 433.200 MHz | 2.0 MSPS | 否 |
| Red | L1 | 广播 + 干扰机 | 432.700 MHz | 3.0 MSPS | 是 |
| Red | L2 | 广播 + 干扰机 | 432.900 MHz | 2.5 MSPS | 是 |
| Red | L3 | 广播 = 干扰机 | 433.200 MHz | 2.0 MSPS | 否 |
| Blue | L1 | 广播 + 干扰机 | 434.420 MHz | 3.0 MSPS | 是 |
| … | … | … | … | … | … |
...
| 参数 | 位置 | 说明 |
|---|---|---|
| `rx_gain_db` | `config.json` | 按第 7 节方法调整至 IQ RMS 0.05~0.30 |
| `pluto_uri` | `config.json` | 更换 RX 设备后可改为 `rtl=0` 或新 IP |
| `rx_buf_size` | `config.json` | RTL-SDR 建议 `131072`，Pluto 建议 `262144` |
...
```json
// config.json
"tx_attenuation_db": 10   // 增大此值 = 降低发射功率（0 = 最大，89.75 = 最小）
```
...
| `DAC_FULL_SCALE` | `2047` | 可调（不超过 2047） | AD9363 12-bit DAC 满量程 |
| `pluto_uri` | `"ip:192.168.2.1"` | 按实际 IP 修改 | `config.json` |
| `rx_gain_db` | `30` dB（默认） | **需按实际调整** | `config.json` |
| `rx_buf_size` | `262144` | 可改（2 的幂） | `config.json` |
| `DIGITAL_LPF_CUTOFF` | `FSK_BW/2 × 1.2 = 324 kHz` | 自动计算 | `config_manager.py` |
